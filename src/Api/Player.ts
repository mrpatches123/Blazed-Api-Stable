...
export class Player {
    ...
    async getLocation(): Promise<Vec3> {
        return {
     ...
     */
    async getScore(objective: string): Promise<number> {
     ...
     */
    message(message: string | number | symbol) {
     ...
     */
    async runCommandAsync(command: string): Promise<boolean> {
    ...
    runCustomCommand(command: string): void {
        ...
        if (!data) return
        try { data.callback({ player: this, args }) } catch { }
    }
    /**
